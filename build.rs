...
fn build_test_helper() {
	...
	let target = env::var("TARGET").unwrap_or_else(|_| "i686-pc-windows-msvc".to_string());
	...
	let status = Command::new("rustc")
		.args(&[test_helper_src, "-o", output_path.to_str().unwrap()])
		.status();

	match status {
		Ok(exit_status) if exit_status.success() => {
			println!(
				"cargo:warning=Successfully built test_helper at: {}",
				output_path.display()
			);
		}
		Ok(_) => {
...
fn main() {
	...
	fs::write("resources/resources.rc", updated_content).expect("Failed to write resources.rc");
